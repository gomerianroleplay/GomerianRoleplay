...
CMD:aduty(playerid, params[])
{
    ...
    if (!AccountData[playerid][pAdminDuty])
    {
        ...
        SendAdminMessage(X11_TOMATO_1, "* %s is now duty as an admin.", ReturnName(playerid, 1));
    }
    else
    {
        ...
        SendAdminMessage(X11_TOMATO_1, "* %s is no longer on admin duty.", ReturnName(playerid, 1));
    }
    return 1;
}
...
CMD:send(playerid, params[])
{
    ...
    if (sscanf(params, "us[128]", userid, type))
    {
        ...
        SendClientMessage(playerid, X11_YELLOW_2, "[LOCATION]:"WHITE" ls, lv, sf, mechanic, smb, bank, unitybus, blackmarket, fish, miner");
    ...
    else if (!strcmp(type, "unitybus")) SendPlayerPos(userid, 1789.9645, -1911.4059, 13.5041, 326.1944);
